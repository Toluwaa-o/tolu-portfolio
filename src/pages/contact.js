import illustration from '../images/contact.png'
...
export default function Contact() {
  ...
  const sendDetails = (e) => {
    ...
    axios.post('https://portfolio-api-y4pv.onrender.com/send', data, {
        headers: {
    ...
    .then(res => {
      setErr('sent')
      return setData({name: '', email: '', message: ''})
    })
    .catch (error => setErr('Something went wrong.Please try again later.'))
  }
    ...
    <section id='contact' className='contact'>
      ...
      <form onSubmit={sendDetails}>
        <fieldset>
          ...
          <input onChange={updateData} type='text' name='name' placeholder='Full Name' aria-label='Full name' value={data.name} />
        </fieldset>

        <fieldset>
          <legend>Email:</legend>
          <input onChange={updateData} value={data.email} type='email' name='email' placeholder='Email Address' aria-label='Email' />
        </fieldset>

        <fieldset>
          <legend>Message:</legend>
          <textarea onChange={updateData} value={data.message} name='message' placeholder='Message' aria-label='Message'></textarea>
        </fieldset>

        {err && <p style={{color: err === 'sent' ? 'green' : 'red', display: 'flex'}}>{err}{err === 'sent' && <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style={{marginLeft: '.2em', fill: 'green'}}><path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm-1.999 14.413-3.713-3.705L7.7 11.292l2.299 2.295 5.294-5.294 1.414 1.414-6.706 6.706z"></path></svg>}</p>}
